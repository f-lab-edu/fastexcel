 ...
 */
public class EighthMultiSheetParallelWriter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path resultsPath = Paths.get(OUTPUT_DIR, "flush_multi_sheet_queue_results_" + timestamp + ".txt");
        ...
        // Flush() 사용 스트리밍 성능 테스트
        System.out.println("\nFlush() 스트리밍 테스트 시작");
        createOutputDirectory();
        String ts = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        String fileName = OUTPUT_DIR + "/flush_test_" + ts + ".xlsx";

        long start = System.currentTimeMillis();
        long initialMem = getUsedMemory();

        try (FileOutputStream fos = new FileOutputStream(fileName);
             Workbook workbook = new Workbook(fos, "FlushTest", "1.0")) {
            Worksheet sheet = workbook.newWorksheet("Data");
            Random random = new Random();

            // 헤더
            for (int c = 0; c < COLUMN_COUNT; c++) {
                sheet.value(0, c, "Col" + (c + 1));
                sheet.width(c, 15);
            }
            ...
            for (int r = 1; r <= ROW_COUNT; r++) {
                for (int c = 0; c < COLUMN_COUNT; c++) {
                    if (random.nextBoolean()) {
                        sheet.value(r, c, random.nextDouble() * 10000);
                    } else {
                        sheet.value(r, c, STRING_VALUES[random.nextInt(STRING_VALUES.length)]);
                    }
                }
                if (r % 20_000 == 0) {
                    sheet.flush(); // 메모리 방출
                    System.out.println("  플러시 실행: " + r + "행, 메모리 사용: " + (getUsedMemory() - initialMem) / (1024 * 1024) + "MB");
                }
                written += COLUMN_COUNT;
                if (r % 100_000 == 0) {
                    System.out.println("진행: " + r + "/" + ROW_COUNT + " (" + String.format("%.2f", r * 100.0 / ROW_COUNT) + "%)" );
                }
            }
        }
        long end = System.currentTimeMillis();
        long usedMem = (getUsedMemory() - initialMem) / (1024 * 1024);
        System.out.println("완료. 시간: " + (end - start) + "ms, 사용 메모리: " + usedMem + "MB");
    }
    ...
    private static TestResult runTest(boolean useMultiSheet, int threadCount, String filenameSuffix) throws Exception {
        ...
        OutputStream os = new BufferedOutputStream(new FileOutputStream("large_file.xlsx"));

        Workbook workbook = new Workbook(os, "ExcelTest", "1.0");
                ...
                for (int r = 1; r <= ROW_COUNT; r++) { // r은 1부터 시작하는 실제 행 번호
                    for (int c = 0; c < COLUMN_COUNT; c++) {
                        ...
                        if (currentQueueSize > QUEUE_THRESHOLD) {
                            while (dataQueue.size()> QUEUE_THRESHOLD / 2) {
                                Thread.sleep(10); // 백프레셔: 큐가 너무 크면 생산자 대기
                            }
                        }
                    }
                    if (r % 100_000 == 0) {
                        System.out.println("  데이터 생성 진행: " + r + "/" + ROW_COUNT + " 행, 현재 큐 크기: " + dataQueue.size());
                    }
                }
                producerFinished.set(true);
                System.out.println("  데이터 생성 완료. 총 " + (ROW_COUNT * COLUMN_COUNT) + " 셀 생성됨. 최종 큐 크기: " + dataQueue.size());
            } catch (Exception e) {
                e.printStackTrace();
            } 
        });
        ...
        Thread monitorThread = new Thread(() -> {
            try {
                while (writerLatch.getCount() > 0) {
                    System.out.println("    작업 진행 중... 큐 크기: " + dataQueue.size() +
